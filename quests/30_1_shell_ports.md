...
# 셸 스크립트
  ...
  4 port_in_use=0
  5 
  6 echo "포트 사용 상태 확인 중..."
  7 echo
  8 
  9 for port in "$@"
 10 do      
 11         if ss -tulnp | grep -q ":"${port}" " ; then
 12                 echo "포트 "${port}": ✓ 사용 중"
 13                 port_in_use=$(( port_in_use + 1 ))
 14         else    
 15                 echo "포트 "${port}": ✗ 사용 안함"
 16         fi
 17         
 18 done
 19 
 20 echo
 21 echo "총 "${total}"개 포트 중 "${port_in_use}"개가 사용 중입니다."
```

```shell
# 스크립트 실행
[jeongho@192.168.0.47 ~/quest/30_1_quest]$ ./check_ports.sh 7111 7122 22 333333 51694 23
포트 사용 상태 확인 중...
